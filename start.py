...
def send_telegram_message(message):
    ...
    try:
        ...
        # Добавляем IP/MAC (best-effort)
        try:
            from telemetry.telemetry_manager import get_network_info
            network = get_network_info()
            if isinstance(message, str):
                if network.get("ip") and "IP:" not in message:
                    message += f"\n🌐 IP: {network['ip']}"
                if network.get("mac") and "MAC:" not in message:
                    message += f"\n🔌 MAC: {network['mac']}"
        except Exception:
            pass
...
def send_error_to_telegram(exc_type=None, exc_value=None, exc_traceback=None):
    ...
    try:
        ...
        error_message = f"⚠️ Критическая ошибка в Extreme!\n\n"
        ...
        try:
            send_telegram_message(f"⚠️ Произошла ошибка в Extreme, но не удалось получить детали: {str(e)}")
        except:
            pass
